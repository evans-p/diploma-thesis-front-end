...
import "./SelectInputBlockForm.css";
...
class SelectInputBlockForm extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      // The block' image.
      ...
      // A boolean that indicates whether the block's popover is open
      // or not.
      popoverOpen: false,
    };
    // BINDING
  ...
  /**A method to manage the event of adding an option (an element of
   * the "hasOptions" state variable). Adds a new element at the start
   * of "hasOptions" state variable*/
   ...
   * the "hasOptions" state variable). Deletes an element from "hasOptions"
   * variable state.*/
  deleteOption(idx) {
    this.setState((currentState) => {
      return {
   ...
   * "error" state object that were corrected, and after that, call "setSelectInputBlock"
   * method, to apply the changes to the parent Component(SelectInputBlock), then display
  ...
  handleSubmit(e) {
    ...
    this.props.setSelectInputBlock({
  ...
  render() {
    return (
      <form className="SelectInputBlockForm" onSubmit={this.handleSubmit}>
...
export default withSnackbar(SelectInputBlockForm);
